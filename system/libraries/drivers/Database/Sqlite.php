 ...
 */
class Database_SQLite_Driver extends Database_Driver {
	...
	public function __construct($config)
	{
		...
		Kohana::log('debug', 'SQLite Database Driver Initialized');
	}
	...
	{
		is_resource($this->link) and sqlite_close($this->link);
	}
	...
	public function connect()
	{
		...
		$connect = ($this->db_config['persistent'] == TRUE) ? 'sqlite_popen' : 'sqlite_open';
		...
		if ($this->link = $connect($database, 0666, $sqliteerror))
		{
	...
	public function query($sql)
	{
		...
		if ($this->db_config['cache'] AND ! preg_match('#\b(?:INSERT|UPDATE|REPLACE|SET)\b#i', $sql))
		{
			...
			if ( ! isset(self::$query_cache[$hash]))
			{
			  ...
			  self::$query_cache[$hash] = new SQLite_Result(sqlite_query($this->link, $sql), $this->link, $this->db_config['object'], $sql);
			}
...
class SQLite_Result extends Database_Result {
	...
	public function __construct($result, $link, $object = TRUE, $sql)
	{
		...
		{
			$this->current_row = 0;
			$this->total_rows  = sqlite_num_rows($this->result);
			$this->fetch_type = ($object === TRUE) ? 'sqlite_fetch_object' : 'sqlite_fetch_array';
		}
		elseif (is_bool($result))
		{
			...
			{
				// SQL error
				throw new Kohana_Database_Exception('database.error', sqlite_last_error($link).' - '.$sql);
			}
			else
			{
				// Its an DELETE, INSERT, REPLACE, or UPDATE query
				//$this->insert_id  = sqlite_insert_id($link);
				$this->total_rows = sqlite_fetch_all($link);
			}
		}
	...
	public function __destruct()
	{
		...
		{
			sqlite_free_result($this->result);
		}
	}
	...
	public function result($object = TRUE, $type = MYSQL_ASSOC)
	{
		...
		if ($this->fetch_type == 'sqlite_fetch_object' AND $object === TRUE)
		{
	...
	{
		$field_names = array();
		while ($field = sqlite_fetch_object($this->result))
		{
	...
	{
		if ($this->offsetExists($offset) AND sqlite_seek($this->result, $offset))
		{
